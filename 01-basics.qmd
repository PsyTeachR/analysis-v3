...
<!-- -->
```
i)  sub-item 1 (with only 2 spaces) A. sub-sub-item 1 (with an additional 2 spaces from the last indent)
:::
...
5.  list item 1
    ...
    b)  sub-item labelled "i)"
        C)  sub-item labelled "C)"
        D)  sub-item labelled "Z)"
6.  list item 7
...
The data we will be using in the upcoming lab activities is from a study conducted in Canada by Binfet et al. (2021).
...
Open the data files, look at the codebook, and perhaps skim over the original Binfet study (methods in particular) to see what kind of measures they used.
